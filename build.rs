...
fn main() {
    ...
    if target.contains("rumprun") || target.contains("bitrig") ||
       target.contains("openbsd") || target.contains("msvc") ||
    ...
    let ar = cc2ar(compiler.path(), &target).unwrap();
    ...
    run(&mut cmd);
    let mut make = Command::new(make(&host));
